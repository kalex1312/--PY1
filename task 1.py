...
class Furniture:
    ...
    def comfort_sit(self):
        print ("Комфортно сидеть" if self._width <= 0.8 else "Нужны дополнителеные подушки")
        """
...
class Sofa (Furniture):
    ...
    def comfort_lay(self):
        print ("Комфортно лежать" if self._length >= 1.8 else "Нужно подгибать ноги")
        """
...
class Armchair (Furniture):
    ...
    def __str__(self):
        return f"{self._color} {self._material} кресло с подлокотниками" if \
                                                                                {self._armrest} == True else f"{self._color} {self._material} кресло"
